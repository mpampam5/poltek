  ...
  <div class='box-header with-border'>
    <h3 class='box-title'>Modul <?=ucfirst($layout_title)?></h3>
      <div class='box-tools pull-right'>
        <a href='<?=site_url(config_item("cpanel")."dosen/tambah")?>' id='tambah' class='btn btn-success btn-sm'> Tambah</a>
      </div>
  </div>
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                var t = $("#mytable").dataTable({
                    ...
                    processing: true,
                    serverSide: true,
                    ajax: {"url": "<?=base_url(config_item("cpanel"))?>/dosen/json", "type": "POST"},
                    columns: [
                        {"data": "id_pegawai",
                         "orderable": false,
                         "visible":false
                        ...
                        "orderable": false,
                         render:function(data,type,row,meta)
                         {
                           if (data=="" || data==null) {
                             ...
                             var image = "thumbs/"+data;
                           }
                           var custom = '<a href="<?=base_url()?>temp/upload/'+image+'" target="_blank"><img class="img img-responsive img-thumbnail" style="width:100px;height:100px;" src="<?=base_url()?>temp/upload/'+image+'"></a>';
                           return custom;
                         }
                        },
                        {"data": "prodi",
                        ...
                        {"data": "nama",
                         render:function(data,type,row,meta)
                         {
                           var custom = 'NIDN&nbsp;&nbsp;: '+row.nidn+'</br>NAMA&nbsp;: '+data+'</br>'
                                        +'<a href="#">'+row.prodi+'<a>';
                           return custom;
                         }
                        },
           ...
           //     $('#ModalHeader').html('Tambah Data Dosen');
           ...
           //   });
             ...
             //     $('.modal-dialog').removeClass('modal-sm');
             //     $('.modal-dialog').addClass('modal-md');
             //     $('#ModalHeader').html('Edit Data Dosen');
             //     $('#ModalContent').load($(this).attr('href'));
             //     $('#ModalGue').modal('show');
             //   });
           //
